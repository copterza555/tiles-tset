...
export default function Chapterkhloy1() {
  const location = useLocation();
  const navigate = useNavigate();
  const { score, total } = location.state || { score: 0, total: 3 }; // ✅ ดึงคะแนนจาก state
  ...
  return (
    <>
      ...
      <div style={styles.scoreBox}>
        <p style={styles.scoreText}>{score} / {total}</p>
      </div>

      <div style={styles.clickableIcon} onClick={() => navigate("/Startkhloylesson1")}>
          <img src={unlockIcon} alt="ปลดล็อค" style={styles.unlockIcon} />
        </div>

      {/* ✅ แสดงเครื่องดนตรีที่ล็อค (5 อัน) และ ปลดล็อค (1 อัน) */}
      <div>
        <img src={lockIcon} alt="ล็อค 1" style={styles.lockIcon1} />
        <img src={lockIcon} alt="ล็อค 2" style={styles.lockIcon2} />
